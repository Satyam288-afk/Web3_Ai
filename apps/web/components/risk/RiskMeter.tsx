  ...
  return (
    <div className={cn("rounded-lg border bg-ink p-5 shadow-lift", riskColor(level))}>
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <div className="text-sm font-medium text-cyan-100">Risk score</div>
          <div className="mt-1 text-xs text-cyan-100/70">{riskLabels[level]}</div>
        </div>
        <span className="rounded-md border border-current/30 px-2 py-1 text-xs font-semibold">{level}</span>
      </div>
      <div className="text-6xl font-semibold text-white">{clamped}<span className="ml-1 text-base font-medium text-cyan-100/60">/100</span></div>
